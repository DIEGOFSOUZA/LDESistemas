inherited Frm_Produto: TFrm_Produto
  ...
  inherited cds: TClientDataSet
    CommandText = 
      'SELECT a.*,'#13#10'U.NOME DESCRI_UNIDADE,x.NOME CONV_DESCRIUNIDADE,'#13#10'G' +
      '.DESCRI GRUPO,S.DESCRI SUBGRUPO,N.DESCRICAO DESCRI_NCM '#13#10'FROM PR' +
      'ODUTO a'#13#10'LEFT OUTER JOIN UNIDADE U ON (U.CODIGO = a.COD_UNIDADE)' +
      ' '#13#10'LEFT OUTER JOIN UNIDADE X ON (X.CODIGO = a.CONV_UNIDADE)'#13#10'LEF' +
      'T OUTER JOIN GRUPO_PROD G ON (G.CODIGO = a.COD_GRUPO) '#13#10'LEFT OUT' +
      'ER JOIN SUBGRUPO_PROD S ON (S.CODIGO = a.COD_SUBGRUPO) '#13#10'LEFT OU' +
      'TER JOIN NCM N ON (N.NCM = a.COD_NCM)  '#13#10'WHERE 1 = 2'
