...
function FormatarValor(aValue: string): string;

implementation

uses
  Vcl.StdCtrls, ACBrUtil;
...
var
  ...
  TextVal: string;
begin
  AValor := StrToIntDef(OnlyNumber(aValue), 0);
  TextVal := FormatFloatBr(AValor / 100, FloatMask(2, True));
  TextVal := 'R$ ' + TextVal;
