...
begin
  ...
  if lLimite > 0 then
    ...
    begin
      Result := False;
      TMensagem.Informacao('Pagamento não pode ser realizado.' + #13#10 +
                 'Cliente tem limite de ' + FormatFloatBr(lLimite) +
                 ' para compras no Crediário. Se necessário alterar limite no cadastro do cliente.'
                  );
    end;
end;
...
begin
  ...
  if (length(vlrS) > 15)
     then begin
            Extenso := 'Erro: valor superior a 999 trilhões.';
            exit;
          end;
  ...
  if (snrCGC <> '  .   .   /    -') and (snrCGC <> '') then BEGIN
        try
         ...
         if  snrcgc <> wcgccalc then
           begin
             TMensagem.Atencao('C.N.P.J. Inválido !');
        ...
        begin
          MessageDlg('Valor informado não é válido !',mtwarning,[mbok],0);
...
begin
 ...
 if (snrcpf <> '   .   .   -  ') and (snrcpf <> '') then Begin
        ...
        if  snrcpf <> wcpfcalc then begin
             TMensagem.Atencao('C.P.F. Inválido !');
             vercpf := false;
           end
        else
             vercpf := true;
        except on econverterror do begin
             TMensagem.Atencao('Valor informado não é válido !');
             vercpf := false;
        end
        end
    END
...
{Verifica se um ano como string é bissexto}
{Qualquer ano < 20 será considerado como 2000 + ano}
function Bissexto(Ano: string): Boolean;
var
  Numero: Integer;
begin
...
begin
  if pValor = '' then
  begin
    TMensagem.Atencao('Valor não informado.');
    Result := '0';
    Exit;
  end;
