...
ALTER TABLE EMPRESA
    ...
    ADD RASTREABILIDADE_PRODUCAO BOOLEAN DEFAULT FALSE NOT NULL;

ALTER TABLE LOTE_ITENS
    ADD QTDE_DISPONIVEL NUMERIC(15,3);

CREATE TABLE PDV_LOTE (
    ID INTEGER GENERATED BY DEFAULT AS IDENTITY,
    PDV_TIPO CHAR(1) NOT NULL,
    PDV_ID INTEGER NOT NULL,
    PDV_ORDEM INTEGER NOT NULL,
    LOTE_ID INTEGER NOT NULL,
    QTDE NUMERIC(15,3) NOT NULL);

ALTER TABLE PDV_LOTE
ADD CONSTRAINT PK_PDV_LOTE
PRIMARY KEY (ID);

ALTER TABLE PDV_LOTE
ADD CONSTRAINT FK_PDV_LOTE_1
FOREIGN KEY (PDV_ID,PDV_TIPO,PDV_ORDEM)
REFERENCES PDV_ITENS(ID,TIPO,ORDEM);

ALTER TABLE PDV_LOTE
ADD CONSTRAINT FK_PDV_LOTE_2
FOREIGN KEY (LOTE_ID)
REFERENCES LOTE_ITENS(ID);

ALTER TABLE LOTE_ITENS
ADD CONSTRAINT FK_LOTE_ITENS_2
FOREIGN KEY (CODPRO)
REFERENCES PRODUTO(CODIGO)
ON DELETE CASCADE
ON UPDATE CASCADE;
...
CREATE OR ALTER procedure PRO_GET_UNIDADE (
    IDPRODUTO integer not null)
returns (
    UND_CODIGO integer,
    UND_SIGLA varchar(10),
    UND_FRACIONADA_CODIGO integer,
    UND_FRACIONADA_SIGLA varchar(10))
as
begin
  /* Procedure Text */
  for select P.COD_UNIDADE, U1.SIGLA, P.CONV_UNIDADE, U2.SIGLA
      from PRODUTO P
      left join UNIDADE U1 on (U1.CODIGO = P.COD_UNIDADE)
      left join UNIDADE U2 on (U2.CODIGO = P.CONV_UNIDADE)
      where P.CODIGO = :IDPRODUTO
      into :UND_CODIGO, :UND_SIGLA, :UND_FRACIONADA_CODIGO, :UND_FRACIONADA_SIGLA
  do
  begin
    suspend;
  end
...
CREATE INDEX FABRICANTE_LOTE_IDXLOTE
ON FABRICANTE_LOTE (LOTE);

COMMENT ON COLUMN LOTE_ITENS.QTDE IS
'QTDE INFORMADA PELO USUARIO';

COMMENT ON COLUMN LOTE_ITENS.QTDE_FECHADA IS
'QTDE CONVERTIDA PARA BAIXA DE ESTOQUE';

SET TERM ^ ;

CREATE OR ALTER trigger tri_bi_receber_parcial for pdv_receber_parcial
...
GRANT SELECT, INSERT, UPDATE, DELETE, REFERENCES ON PDV_LOTE TO R_SISTEMA;
...
GRANT EXECUTE ON PROCEDURE PRO_GET_UNIDADE TO R_SISTEMA;
